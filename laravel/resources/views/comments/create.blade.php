...
<div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    <div class="bg-dark-800 rounded-xl shadow-lg p-6">
        <h1 class="text-2xl font-bold mb-6">Create New Comment</h1>
        
        <form action="{{ route('comments.store') }}" method="POST">
            @csrf
            <input type="hidden" name="commentable_type" value="{{ request()->query('commentable_type') }}">
            <input type="hidden" name="commentable_id" value="{{ request()->query('commentable_id') }}">
            
            <div class="mb-6">
                <label for="content" class="block text-gray-300 mb-2">Your Comment</label>
                <textarea 
                    name="content" 
                    id="content" 
                    rows="6"
