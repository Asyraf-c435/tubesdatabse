...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    <div class="bg-dark-800 rounded-xl shadow-lg p-6">
        <h1 class="text-3xl font-bold mb-6">Edit Lesson: {{ $lesson->name }}</h1>
        
        <form action="{{ route('lessons.update', [$course->slug, $lesson->slug]) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            ...
            <div class="mb-6">
                ...
                @if($lesson->image_link)
                    <img 
                        src="{{ $lesson->image_link }}" 
                        alt="{{ $lesson->name }}" 
                        class="max-w-xs max-h-48 rounded-lg mb-4"
                    >
                @else
                    <p class="text-gray-400">No image set</p>
                @endif
